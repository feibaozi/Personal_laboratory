...
export function configureAI(config: {
  ...
  if (config.enabled !== undefined) aiEnabled = config.enabled;
  if (config.qwenApiKey !== undefined) qwenApiKey = config.qwenApiKey;
  if (config.qwenModel !== undefined) qwenModel = config.qwenModel;
  if (config.deepseekApiKey !== undefined) deepseekApiKey = config.deepseekApiKey;
  if (config.deepseekModel !== undefined) deepseekModel = config.deepseekModel;
}
...
export async function analyzeGarment(imageBase64: string): Promise<Record<string, unknown> | null> {
...
export async function getRecommendations(context: {
  garments: { id: string; name: string; category: string; colors: string; style: string | null; seasons: string; occasions: string }[];
  occasion?: string;
  weather?: string;
  styleDescription?: string;
}): Promise<{ outfits: { garmentIds: string[]; reason: string; score: number }[]; overallTip?: string } | null> {
  ...
  try {
    ...
    const garmentList = context.garments.map((g) =>
      `[${g.id}] ${g.name} | 类别:${g.category} | 颜色:${g.colors} | 风格:${g.style || '未知'}`
    ).join('\n');
