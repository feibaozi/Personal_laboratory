...
exports.analyzeNote = analyzeNote;
exports.inspectFramework = inspectFramework;
exports.summarizeConversation = summarizeConversation;
exports.aiSearch = aiSearch;
exports.getPracticeReminder = getPracticeReminder;
exports.generateDomainFramework = generateDomainFramework;
exports.refineFramework = refineFramework;
...
async function callLLM(systemPrompt, userMessage, maxTokens = 2000) {
    const endpoint = (0, database_1.getConfig)("api_endpoint") || "https://api.openai.com/v1/chat/completions";
    const apiKey = (0, database_1.getConfig)("api_key") || "";
    const model = (0, database_1.getConfig)("api_model") || "gpt-4o-mini";
    if (!apiKey) {
...
async function callLLMWithRetry(systemPrompt, userMessage, maxTokens = 2000) {
    let lastError = null;
...
async function analyzeNote(content) {
    const frameworkContext = (0, notes_1.getFrameworkContextForAI)();
...
${content}`;
    ...
    return {
        ...
        relatedSuggestions: (parsed.related_suggestions || []).map((s) => ({
            title: s.title || "",
            content: s.content || "",
...
请从中提取关键实践经验。`;
    ...
    return {
        keyInsights: (parsed.key_insights || []).map((i) => ({
            title: i.title || "",
            content: i.content || "",
...
async function aiSearch(naturalLanguageQuery) {
    const frameworkContext = (0, notes_1.getFrameworkContextForAI)();
    const userMessage = `当前框架结构如下（节点ID > 路径）：
${frameworkContext}
...
async function getPracticeReminder(projectDescription) {
    const frameworkContext = (0, notes_1.getFrameworkContextForAI)();
    const userMessage = `当前框架结构如下（节点ID > 路径）：
${frameworkContext}
...
请给出实践提醒，重点推荐 warning 和 principle 类型的节点。`;
    ...
    return {
        ...
        principles: (parsed.principles || []).map((p) => ({
...
async function generateDomainFramework(domainDescription) {
...
async function refineFramework(currentFramework, userFeedback) {
