...
class JDCollector(BaseCollector):
    ...
    def __init__(self):
        ...
        self._platform_headers = {
            ...
            "Origin": "https://waimai.jd.com",
        }

    async def collect_shop_menu(self, shop_url: str) -> CollectionResult:
        try:
            html = await self._page_request(shop_url, wait_ms=4000)
            ...
            logger.error("JD collection failed: %s", e)
    ...
    def _parse_products(self, soup: BeautifulSoup) -> list:
        ...
        for item in items[:50]:
            name_el = (
                item.select_one('.name, .title, .goods-name')
                or item.select_one('[class*="name"]')
            )
            price_el = (
                item.select_one('.price, .goods-price, .sale-price')
    ...
    def _parse_delivery(self, soup: BeautifulSoup) -> dict:
        ...
        if time_el:
            ...
            if len(nums) >= 2:
                ...
                delivery["time_min"] = int(nums[0])
                delivery["time_max"] = int(nums[0]) + 15
    ...
    async def _collect_shops_crawler(self, location: dict) -> CollectionResult:
        ...
        url = f"https://waimai.jd.com/search?keyword={keyword}"
        html = await self._page_request(url, wait_ms=4000)
        soup = await self._parse_page(html)
        shops = []
        shop_els = soup.select('[class*="shop"], [class*="store"], [class*="merchant"]')
    ...
    async def _collect_products_crawler(self, shop_id: str) -> CollectionResult:
        url = f"https://waimai.jd.com/shop/{shop_id}"
    ...
    async def _collect_coupons_crawler(self) -> CollectionResult:
        try:
            html = await self._page_request(self.base_url, wait_ms=4000)
            soup = await self._parse_page(html)
            coupons = self._parse_coupons(soup)
            if coupons:
                return CollectionResult(
            ...
            logger.error("JD coupons crawler failed: %s", e)
            return CollectionResult(
                success=False,
