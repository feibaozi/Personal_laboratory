...
class DouyinCollector(BaseCollector):
    ...
    async def collect_shop_menu(self, shop_url: str) -> CollectionResult:
        try:
            html = await self._page_request(shop_url, wait_ms=5000)
            ...
            logger.error("Douyin collection failed: %s", e)
    ...
    def _parse_shop_info(self, soup: BeautifulSoup) -> dict:
        ...
        selectors = [
            '.shop-name', '.store-name', '.poi-name', 'h1',
            '[class*="shop-name"]', '[class*="store-name"]',
            '[class*="poi-name"]', '.title',
        ]
    ...
    def _parse_products(self, soup: BeautifulSoup) -> list:
        ...
        item_selectors = [
            '.food-item', '.dish-item', '.product-item', '.goods-item',
            '[class*="food"]', '[class*="goods"]', '[class*="product"]',
            '[class*="dish"]',
        ]
        items = []
        ...
        for item in items[:50]:
            name_el = (
                item.select_one('.name, .title, .goods-name, .product-name')
                or item.select_one('[class*="name"]')
            )
            price_el = (
                item.select_one('.price, .goods-price, .sale-price, .product-price')
    ...
    def _parse_delivery(self, soup: BeautifulSoup) -> dict:
        ...
        fee_el = soup.select_one(
            '[class*="delivery"], [class*="shipping"], [class*="freight"]'
        )
        if fee_el:
        ...
        time_el = soup.select_one('[class*="time"], [class*="arrive"]')
        if time_el:
            ...
            if len(nums) >= 2:
                ...
                delivery["time_min"] = int(nums[0])
                delivery["time_max"] = int(nums[0]) + 20

        min_el = soup.select_one('[class*="min"], [class*="minimum"]')
        if min_el:
    ...
    def _make_fallback_result(self, error: str) -> CollectionResult:
        return CollectionResult(
            ...
            data={
                ...
                "message": "自动采集失败，抖音外卖页面通常需要登录态，建议使用截图OCR",
    ...
    async def _collect_shops_crawler(self, location: dict) -> CollectionResult:
        ...
        html = await self._page_request(url, wait_ms=5000)
        soup = await self._parse_page(html)
        shops = []
        shop_els = soup.select('[class*="shop"], [class*="store"], [class*="poi"], [class*="merchant"]')
        for el in shop_els[:20]:
            name_el = el.select_one('[class*="name"], [class*="title"], h3, h4')
            rating_el = el.select_one('[class*="rating"], [class*="score"], [class*="star"]')
            rating = 0.0
            if rating_el:
                nums = re.findall(r'(\d+\.?\d*)', rating_el.text.strip())
                if nums:
                    try:
                        rating = float(nums[0])
                        if rating > 5:
                            rating = rating / 10 if rating <= 50 else 0.0
                    except ValueError:
                        pass
            shops.append({
                "shop_id": "",
    ...
    async def _collect_products_crawler(self, shop_id: str) -> CollectionResult:
        url = f"https://www.douyin.com/user/self?modal_id={shop_id}"
    ...
    async def _collect_coupons_crawler(self) -> CollectionResult:
        try:
            html = await self._page_request(self.base_url, wait_ms=5000)
            soup = await self._parse_page(html)
            coupons = self._parse_coupons(soup)
            if coupons:
                return CollectionResult(
            ...
            logger.error("Douyin coupons crawler failed: %s", e)
            return CollectionResult(
                success=False,
